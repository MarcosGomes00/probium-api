...
LIGAS =[
    ...
    "soccer_netherlands_eredivisie", "soccer_brazil_campeonato", "soccer_brazil_copa_do_brasil",      
...
def salvar_aposta_sistema(bet_data):
    # Salva no Banco de Dados (Novo Padrão)
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        cursor.execute("""
    ...
    # Salva no JSON (Para manter seu Dashboard funcionando)
    try:
...
# ==========================================
# 2. MOTOR DE LINHAS ASIÁTICAS E PLAYER PROPS
# ==========================================
def buscar_valor_linhas_asiaticas(pinnacle, bet365, nome_mercado):
    ...
    if pin_market and b365_market and len(pin_market["outcomes"]) == 2:
        # Pega as duas pontas da Pinnacle para tirar a margem
        p_out1 = pin_market["outcomes"][0]
        p_out2 = pin_market["outcomes"][1]
        
        # Só calcula se a Pinnacle tiver os valores de ponto (ex: -1.5, Over 2.5)
        if "point" not in p_out1: return oportunidades
        
        margin = (1/p_out1["price"]) + (1/p_out2["price"])
        
        # Cruza as linhas com a Bet365
        for b_out in b365_market["outcomes"]:
            # Se a linha (ex: 2.5) e a seleção (ex: Over) baterem com a Pinnacle
...
# ==========================================
# 3. VERIFICAÇÃO AUTOMÁTICA (GREEN/RED)
# ==========================================
def verificar_resultados_automatico():
    global ultima_checagem_resultados
    agora = time.time()
    if agora - ultima_checagem_resultados < 3600: return # Roda a cada 1 hora
        ...
        for esporte in esportes_pendentes:
            ...
            for aposta in pendentes:
                ...
                if "Vitória Casa" in mercado and home_score > away_score: res_final = "GREEN"
                    ...
                    # Resolve os Overs/Unders baseados no placar somado
...
def processar_jogos_e_enviar():
    ...
    print(f"\n🔄[ATUALIZAÇÃO - {agora_br.strftime('%H:%M:%S')}] Escaneando H2H, Asiáticos e Player Props...")

    for liga in LIGAS:
        is_nba = "basketball" in liga
        # Adicionado o mercado de player_points para a NBA!
        ...
        try:
            ...
            for evento in dados:
                ...
                # --- 1. MERCADOS SECUNDÁRIOS: SPREADS E TOTALS (HANDICAP ASIÁTICO E GOLS) ---
                ...
                # --- 4. FILTRAR E ENVIAR A MELHOR OPORTUNIDADE DO JOGO ---
                if not oportunidades: continue
                melhor_op = max(oportunidades, key=lambda x: x[4]) # Pega a de maior EV
                ...
                if 30 <= minutos_faltando <= 120:
                    if jogo_id not in jogos_enviados:
                        ...
                        print(f"🚀 Tip enviada: {mercado_nome} | EV: +{ev_real*100:.2f}%")
...
if __name__ == "__main__":
    ...
    print("✅ Módulos Ativos: Banco de Dados | Linhas Asiáticas (Spreads/Totals) | Player Props NBA | Auto-Green")
    while True:
