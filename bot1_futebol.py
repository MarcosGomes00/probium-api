...
DB_FILE = "probum.db"
SCAN_INTERVAL = 21600 # 6 Horas (Foco em Economia)
...
# EXPANSÃO DE LIGAS (Ligas de Over Gols e Ambas Marcam)
LEAGUE_TIERS = {
    ...
    "soccer_netherlands_eredivisie": 1.1, "soccer_england_efl_championship": 1.1,
    "soccer_mexico_ligamx": 0.9, "soccer_argentina_primeradivision": 0.9,
    "soccer_belgium_first_division": 1.0, "soccer_turkey_super_lig": 1.0,
    "soccer_denmark_superliga": 0.9, "soccer_austria_bundesliga": 0.9
}
...
def salvar_aposta_banco(op, stake):
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        # Adiciona colunas extras se necessário para a nova inteligência do Bot 2
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDENTE', 0, ?, ?, ?)
        """, (id_aposta, 'soccer', f"{op['home_team']} x {op['away_team']}", op['evento']['sport_title'], op['mercado_nome'], op['selecao_nome'], op['odd_bookie'], op['prob_justa'], op['ev_real'], stake, hoje, op.get('odd_pinnacle', 0), op.get('ranking_score', 0)))
        conn.commit()
        conn.close()
    except: pass
...
def validar_futebol(odd, ev, liga):
    if not (1.40 <= odd <= 10.0) or ev > 0.20: return False
    # Filtro mais rigoroso para ligas secundárias
    min_ev = 0.015 if LEAGUE_TIERS.get(liga, 1.0) >= 1.2 else 0.025
    return ev >= min_ev

async def processar_liga_async(session, liga_key, agora_br):
    # BUSCA H2H, BTTS E TOTAL GOLS NA MESMA REQUISIÇÃO (ECONOMIA DE API)
    ...
    for evento in data:
        ...
        for soft_b in bookmakers:
            ...
            for m_key in ["h2h", "btts", "totals"]:
                ...
                if pin_m and soft_m:
                    # Lógica de cálculo de probabilidade justa simplificada para economia
                    margem_pin = sum(1/out["price"] for out in pin_m["outcomes"] if out["price"] > 0)
                    for s_out in soft_m["outcomes"]:
                        # Busca correspondência na Pinnacle
        ...
        if oportunidades_jogo:
            ...
            # Formatação Amigável
            mercado_txt = "Vencedor (1X2)" if melhor["mercado_nome"] == "H2H" else ("Ambas Marcam" if melhor["mercado_nome"] == "BTTS" else "Gols Mais/Menos")
            selecao_txt = melhor["selecao_nome"].replace("Yes", "Sim").replace("No", "Não")
            
            txt = (f"⚽ <b>VALOR ENCONTRADO</b>\n\n🏆 <b>Liga:</b> {melhor['evento']['sport_title']}\n⚔️ <b>Jogo:</b> {melhor['home_team']} x {melhor['away_team']}\n"
                   f"⏰ <b>Horário:</b> {melhor['horario_br'].strftime('%H:%M')}\n\n🎯 <b>Mercado:</b> {mercado_txt}\n👉 <b>Entrada:</b> {selecao_txt}\n"
...
async def loop_infinito():
    while True:
        async with aiohttp.ClientSession() as session:
            ...
            print(f"⚽ Varredura Futebol Iniciada: {agora_br.strftime('%H:%M')}")
